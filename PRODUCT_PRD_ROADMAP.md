...
| M3-06 | 模板 changelog | P1 | `template info` 展示 release 级变更，`upgrade-check` 引导先查看变更 |
...
当前进入 Milestone 3：版本化模板能力。

1. 梳理现有远程 manifest 字段，补齐描述、标签、推荐场景等 v0.5 schema。
2. 完善 `template info` / `template versions` 的文档、示例和失败提示。
3. 新增私有模板仓库使用指南，覆盖 GitHub token、release asset、manifest、缓存与排障。
4. 验证 `--repo`、`TSU_TEMPLATE_REPOSITORY`、`GITHUB_TOKEN` / `GH_TOKEN` 三条私有仓库路径。
5. 继续验证模板 changelog 在私有仓库 release、`template info` 和 `upgrade-check` 中的实际使用体验。
6. 以 release 验证脚本覆盖 Milestone 3 主路径，避免私有仓库工作流回归。
