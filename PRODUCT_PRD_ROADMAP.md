...
> 口径说明：本文档中的“私有化”仅指企业使用自有 GitHub 模板仓库和 Release asset 分发模板，CLI 通过 `--repo` / `TSU_TEMPLATE_REPOSITORY` / `GITHUB_TOKEN` 访问；不规划完整企业私有化平台、后台服务或私有部署版 Tsu 服务。
...
让 Tsu 成为一个“可版本化、可私有模板仓库、可演进”的模板 CLI。

关键目标：

- 支持企业私有模板仓库最佳实践，并稳定 `--repo` / 环境变量 / token 工作流。
- 支持模板升级检查。
- 支持项目健康检查。
- 支持组织级 preset。
- 形成稳定 API 与 CLI 语义。
...
- 根目录 README。
- CLI `--help` / `--version`。
- CLI `templates`、`list`、`template list` 模板列表命令。
- `template info` / `template versions` 查询能力。
- `doctor` / `upgrade-check` 项目检查能力。
...
```json
{
  ...
  "schemaVersion": "0.5",
  "version": "1.1.0",
  "asset": "tsu-templates-v1.1.0.tar.gz",
  "templates": [
    {
      "name": "vue3",
      "title": "Vue 3 Web App",
  ...
  "changelog": [
    "Add private template repository guide.",
    "Add generatedAt to template metadata."
  ]
}
```
...
| M1-02 | CLI `--help` | P0 | 已具备，收尾别名和子命令帮助一致性 |
| M1-03 | CLI `--version` | P0 | 已具备 |
| M1-04 | CLI `templates` | P0 | 已具备，`list` / `template list` 作为兼容别名 |
| M1-05 | init next steps | P0 | 已具备 |
| M1-06 | 模板说明表 | P0 | README / CLI_USAGE 同步 |
| M1-07 | 模板 README | P1 | 下一步模板优化主线，至少覆盖 vue3 / react / mfe / monorepo |
| M1-08 | 生成模板验证脚本 | P1 | 已具备基础验证，后续强化模板运行体验 |
...
状态：功能完成，进入 v0.5.0 发布前验证。

目标：把“模板版本化”做成核心差异化，并把企业私有模板仓库能力收口为可文档化、可验证的 CLI 工作流。

边界：Milestone 3 只支持基于 GitHub 仓库和 Release asset 的私有模板分发；不建设完整企业私有化平台、后台服务或私有部署版 Tsu 服务。

### 交付项

| 编号 | 事项 | 优先级 | 说明 |
| --- | --- | --- | --- |
| M3-01 | 远程 manifest 扩展 | P0 | 已完成：模板描述、标签、推荐场景、schemaVersion |
| M3-02 | `template info` | P0 | 已完成：查看本地和远程模板详情 |
| M3-03 | `template versions` | P1 | 已完成：查看可用模板版本 |
| M3-04 | 私有仓库文档 | P0 | 已完成：企业私有 GitHub Release asset 工作流 |
| M3-05 | 生成元信息 | P1 | 已完成：记录模板来源、版本、仓库和生成时间 |
| M3-06 | 模板 changelog | P1 | 已完成：`template info` 展示 release 级变更，`upgrade-check` 引导先查看变更 |
...
- 用户可以锁定指定模板版本。
- 企业用户能按照文档配置自己的模板仓库。
- 私有仓库文档明确 token 配置、release asset 规范、manifest 规范和常见失败排查。
- 私有仓库能力通过 `--repo`、`TSU_TEMPLATE_REPOSITORY`、`GITHUB_TOKEN` / `GH_TOKEN` 路径验证。

### 发布前剩余工作

- 为 `@tsuz/cli` 和 `@tsuz/template` 准备 v0.5.0 changeset。
- 执行 `pnpm build`、`pnpm lint`、`pnpm test`。
- 执行 `template:release:build --version=0.5.0` 和 quick `validate:template-release`。
- 执行 `npm:release:preflight` 和 `npm:release:pack`。
...
- 不做完整企业私有化平台、后台服务或私有部署版 Tsu 服务。
- “私有化”能力收敛为企业使用自有 GitHub 模板仓库，通过 `--repo`、`TSU_TEMPLATE_REPOSITORY`、`GITHUB_TOKEN` / `GH_TOKEN` 访问私有 Release asset。
- 生成项目记录 `.tsu/template.json` 元信息，包含模板名、版本、来源、仓库和 `generatedAt` 生成时间。
- 模板 changelog 先收敛为 manifest 顶层 release 级 `changelog` 字段，`template info` 展示该字段，`upgrade-check` 引导先查看 changelog 再重新生成并对比变更。

## 17. 下一步建议

当前 Milestone 3 功能已完成，下一步进入 v0.5.0 发布准备。

1. 提交 v0.5.0 changeset，覆盖 `@tsuz/cli` 和 `@tsuz/template` minor 版本。
2. 执行完整 build / lint / test。
3. 用 `template:release:build --version=0.5.0` 和 quick `validate:template-release` 验证模板 release archive。
4. 执行 `npm:release:preflight` 和 `npm:release:pack`。
5. 校验通过后，再决定是否推送分支、合并到 `master` 并触发 npm / template release。
