...
test("runCli initializes python-main projects", async () => {
  ...
  try {
    ...
    assert.match(message, /pip install -e \.\[dev\]/);
    assert.match(await readFile(join(cwd, "auth-service", "README.md"), "utf8"), /Generated by Tsu from the `python-main` template/);
...
test("runCli initializes python-app projects", async () => {
  ...
  try {
    ...
    assert.match(message, /uvicorn app.main:app --reload/);
    assert.match(await readFile(join(cwd, "backend-api", "README.md"), "utf8"), /Generated by Tsu from the `python-app` template/);
