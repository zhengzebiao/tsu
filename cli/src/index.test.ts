...
function assertPythonDeployArtifacts(options: { readme: string; deployWorkflow: string; migrateWorkflow: string; deployCompose: string; infraCompose: string; envDeploy: string; includePrivateKey: boolean }) {
  assertPythonDeployWorkflow(options.deployWorkflow, options.includePrivateKey);
  assertPythonMigrateWorkflow(options.migrateWorkflow);
  assert.match(options.deployCompose, /\$\{DOCKER_IMAGE_NAME\}:\$\{APP_VERSION\}/);
  assert.match(options.deployCompose, /external: true/);
  assert.match(options.deployCompose, /\/health/);
  assert.doesNotMatch(options.deployCompose, /build:/);
  assert.match(options.infraCompose, /postgres:16-alpine/);
  assert.match(options.infraCompose, /redis:7-alpine/);
  assert.match(options.infraCompose, /postgres_data/);
  assert.match(options.infraCompose, /appendonly yes/);
  assert.match(options.envDeploy, /DOCKER_IMAGE_NAME/);
  assert.match(options.envDeploy, /APP_VERSION/);
  assert.match(options.envDeploy, /DATABASE_URL/);
  assert.match(options.envDeploy, /REDIS_URL/);
  assert.match(options.envDeploy, /JWT_PUBLIC_KEY/);
  assert.match(options.envDeploy, /TOKEN_BLACKLIST_PREFIX/);
  assert.match(options.envDeploy, /SESSION_PREFIX/);
  assert.match(options.envDeploy, /DEPLOY_HEALTH_RETRIES/);
  assert.match(options.envDeploy, /DEPLOY_HEALTH_INTERVAL_SECONDS/);
  assert.match(options.envDeploy, /DEPLOY_PUBLIC_HEALTH_URL/);
  assert.match(options.readme, /Release Deploy and Rollback/);
  assert.match(options.readme, /Docker Infra/);
  assert.match(options.readme, /Tag Release/);
  assert.match(options.readme, /Rollback/);
  assert.match(options.readme, /Post-deploy health check/);
  assert.match(options.readme, /Smoke test/);
  assert.match(options.readme, /Backup checklist/);
  assert.match(options.readme, /Multi-service release/);
  assert.match(options.readme, /Logging and monitoring/);
  assert.match(options.readme, /Rollback playbook/);
  assert.match(options.readme, /DEPLOY_PUBLIC_HEALTH_URL/);
  assert.match(options.readme, /X-Request-ID/);
  assert.match(options.readme, /Migration Policy/);
  assert.match(options.readme, /Migrate workflow/);
  assert.match(options.readme, /backup_confirmed = true/);
  assert.match(options.readme, /Seed Policy/);
  assert.match(options.readme, /Product does not auto-run seed/);
  if (options.includePrivateKey) {
    assert.match(options.envDeploy, /JWT_PRIVATE_KEY/);
    assert.match(options.readme, /python-main` owns `JWT_PRIVATE_KEY/);
  } else {
    assert.doesNotMatch(options.envDeploy, /JWT_PRIVATE_KEY/);
    assert.match(options.readme, /python-app` uses only `JWT_PUBLIC_KEY/);
  }
}
...
test("createTemplateInfoMessage reports template details", () => {
  ...
  assert.match(message, /Template: vue3/);
  assert.match(message, /Title: Vue 3 Web App/);
...
test("createTemplateInfoMessage reports MFE template details", () => {
  ...
  assert.match(mainMessage, /Title: React Micro Frontend Host/);
  ...
  assert.match(appMessage, /Title: React Micro Frontend Sub App/);
...
test("createRemoteTemplateInfoMessage reports versioned template details", () => {
  ...
  assert.match(message, /Template: vue3/);
  assert.match(message, /Title: Versioned Vue App/);
...
test("runCli initializes remote templates from direct URLs and reuses cache", async () => {
  ...
  try {
    ...
    assert.deepEqual(requestedUrls, ["https://github.com/company/templates/releases/download/template-v1.2.3/tsu-templates-v1.2.3.tar.gz"]);
    assert.deepEqual(requestedAuthorization, ["Bearer private-token"]);
...
test("runCli initializes remote templates from TSU_TEMPLATE_REPOSITORY", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "quick-start-"));
  const archiveRoot = await mkdtemp(join(tmpdir(), "quick-start-release-"));
  const cacheDir = await mkdtemp(join(tmpdir(), "quick-start-cache-"));
  const originalFetch = globalThis.fetch;
  const originalCacheDir = process.env.TSU_TEMPLATE_CACHE_DIR;
  const originalRepository = process.env.TSU_TEMPLATE_REPOSITORY;
  const archiveName = "tsu-templates-v1.2.3.tar.gz";
  const bundleDir = join(archiveRoot, "tsu-templates-v1.2.3");
  const archivePath = join(archiveRoot, archiveName);
  const requestedUrls: string[] = [];
  ...
  try {
    process.env.TSU_TEMPLATE_CACHE_DIR = cacheDir;
    process.env.TSU_TEMPLATE_REPOSITORY = "company/env-templates";
    await mkdir(join(bundleDir, "react"), { recursive: true });
    await writeFile(join(bundleDir, "manifest.json"), JSON.stringify({ version: "1.2.3", templates: ["react"] }), "utf8");
    await writeFile(join(bundleDir, "react", "package.json"), '{"name":"env-template"}\n', "utf8");
    await execFileAsync("tar", ["-czf", archiveName, "-C", dirname(bundleDir), "tsu-templates-v1.2.3"], { cwd: archiveRoot });
    const archiveBytes = await readFile(archivePath);
    ...
    await runCli(["init", "env-demo", "--template", "react", "--version", "1.2.3"], cwd);

    assert.deepEqual(requestedUrls, ["https://github.com/company/env-templates/releases/download/template-v1.2.3/tsu-templates-v1.2.3.tar.gz"]);
    const metadata = JSON.parse(await readFile(join(cwd, "env-demo", ".tsu", "template.json"), "utf8"));
    assert.equal(metadata.template.repository, "company/env-templates");
  } finally {
    globalThis.fetch = originalFetch;
    restoreEnv("TSU_TEMPLATE_CACHE_DIR", originalCacheDir);
    restoreEnv("TSU_TEMPLATE_REPOSITORY", originalRepository);
    await rm(cwd, { force: true, recursive: true });
    await rm(archiveRoot, { force: true, recursive: true });
    await rm(cacheDir, { force: true, recursive: true });
  }
...
test("runCli reports private template manifest misses with troubleshooting guidance", async () => {
  ...
  try {
    ...
    await mkdir(join(bundleDir, "react"), { recursive: true });
    await writeFile(join(bundleDir, "manifest.json"), JSON.stringify({ version: "1.2.3", templates: ["react"] }), "utf8");
    await writeFile(join(bundleDir, "react", "package.json"), '{"name":"react-only"}\n', "utf8");
    await execFileAsync("tar", ["-czf", archiveName, "-C", dirname(bundleDir), "tsu-templates-v1.2.3"], { cwd: archiveRoot });
    const archiveBytes = await readFile(archivePath);

    globalThis.fetch = async () => new Response(archiveBytes, { status: 200 });

    await assert.rejects(() => runCli(["init", "missing-demo", "--template", "vue3", "--version", "1.2.3", "--repo", "company/templates", "--no-cache"], cwd), /Check manifest\.json and the template directories inside the archive/);
  } finally {
    globalThis.fetch = originalFetch;
    restoreEnv("TSU_TEMPLATE_CACHE_DIR", originalCacheDir);
    await rm(cwd, { force: true, recursive: true });
...
test("initProject writes template files", async () => {
  ...
  try {
    ...
    const metadata = JSON.parse(await readFile(join(result.targetDir, ".tsu/template.json"), "utf8"));
...
test("runCli initializes python-main projects", async () => {
  ...
  try {
    ...
    const deployWorkflow = await readFile(join(cwd, "auth-service", ".github", "workflows", "deploy.yml"), "utf8");
    const migrateWorkflow = await readFile(join(cwd, "auth-service", ".github", "workflows", "migrate.yml"), "utf8");
    const deployCompose = await readFile(join(cwd, "auth-service", "docker-compose.deploy.yml"), "utf8");
    const infraCompose = await readFile(join(cwd, "auth-service", "docker-compose.infra.yml"), "utf8");
    const envDeploy = await readFile(join(cwd, "auth-service", ".env.deploy.example"), "utf8");
    ...
    assertPythonDeployArtifacts({ readme, deployWorkflow, migrateWorkflow, deployCompose, infraCompose, envDeploy, includePrivateKey: true });
    ...
    assert.doesNotMatch(workflow, /deploy-test:|deploy-product:|TEST_DOCKER_REGISTRY_TOKEN|PRODUCT_DOCKER_REGISTRY_TOKEN|PRODUCT_JWT_PRIVATE_KEY/);
    ...
    const authService = await readFile(join(cwd, "auth-service", "app", "services", "auth_service.py"), "utf8");
    ...
    const authRefreshTokenService = await readFile(join(cwd, "auth-service", "app", "services", "refresh_token_service.py"), "utf8");
    assert.match(authRefreshTokenService, /refresh_token_reuse_grace_seconds/);
    assert.match(authRefreshTokenService, /replaced_by/);
    assert.match(authRefreshTokenService, /RefreshTokenReuseError/);
    ...
    assert.match(await readFile(join(cwd, "auth-service", "app", "seed", "__main__.py"), "utf8"), /DEFAULT_ADMIN_PASSWORD = "password123"/);
    ...
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_auth_service.py"), "utf8"), /test_login_uses_db_user_password_and_rbac_claims/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_auth_service.py"), "utf8"), /test_login_failure_logs_reason_without_password/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_auth_service.py"), "utf8"), /test_refresh_reuse_revokes_db_session/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_auth_service.py"), "utf8"), /test_logout_blacklists_jti_revokes_redis_and_db_session/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_logging.py"), "utf8"), /test_security_logs_redact_sensitive_values/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_redis_state_services.py"), "utf8"), /test_blacklist_service_uses_configured_prefix_and_access_token_ttl/);
    assert.match(await readFile(join(cwd, "auth-service", "tests", "test_refresh_token_service.py"), "utf8"), /test_refresh_token_plaintext_is_not_stored_in_redis/);
...
test("runCli initializes python-app projects", async () => {
  ...
  try {
    ...
    const deployWorkflow = await readFile(join(cwd, "backend-api", ".github", "workflows", "deploy.yml"), "utf8");
    const migrateWorkflow = await readFile(join(cwd, "backend-api", ".github", "workflows", "migrate.yml"), "utf8");
    const deployCompose = await readFile(join(cwd, "backend-api", "docker-compose.deploy.yml"), "utf8");
    const infraCompose = await readFile(join(cwd, "backend-api", "docker-compose.infra.yml"), "utf8");
    const envDeploy = await readFile(join(cwd, "backend-api", ".env.deploy.example"), "utf8");
    ...
    assert.match(readme, /GitHub Actions, Secrets, and Environments/);
    assertPythonDeployArtifacts({ readme, deployWorkflow, migrateWorkflow, deployCompose, infraCompose, envDeploy, includePrivateKey: false });
    ...
    assert.match(workflow, /docker-build:/);
    assert.match(workflow, /docker build --tag/);
    assert.doesNotMatch(workflow, /deploy-test:|deploy-product:|TEST_DOCKER_REGISTRY_TOKEN|PRODUCT_DOCKER_REGISTRY_TOKEN|TEST_JWT_PRIVATE_KEY|PRODUCT_JWT_PRIVATE_KEY/);
    assert.match(nginx, /log_format safe_json/);
    assert.doesNotMatch(nginx, /\$http_authorization|\$http_cookie/);
    ...
    const appAuthDeps = await readFile(join(cwd, "backend-api", "app", "deps", "auth.py"), "utf8");
    ...
    assert.match(await readFile(join(cwd, "backend-api", "tests", "test_profile_api.py"), "utf8"), /test_profile_rejects_malformed_roles_claim/);
