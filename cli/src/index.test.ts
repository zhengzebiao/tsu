...
function assertPythonDeployArtifacts(options: { readme: string; deployWorkflow: string; deployCompose: string; infraCompose: string; envDeploy: string; includePrivateKey: boolean }) {
...
test("runCli initializes python-main projects", async () => {
  ...
  try {
    ...
    assertPythonDeployArtifacts({ readme, deployWorkflow, deployCompose, infraCompose, envDeploy, includePrivateKey: true });
...
test("runCli initializes python-app projects", async () => {
  ...
  try {
    ...
    assert.match(readme, /GitHub Actions, Secrets, and Environments/);
    assertPythonDeployArtifacts({ readme, deployWorkflow, deployCompose, infraCompose, envDeploy, includePrivateKey: false });
