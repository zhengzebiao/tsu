...
export const defaultCliEntry = join(repoRoot, "cli", "dist", "index.js");
...
export const mfeAppTemplates = appTemplates.filter((template) => template.name === "mfe-main" || template.name === "mfe-app");

export async function validateGeneratedApps(options = {}) {
  const tempRoot = options.tempRoot ?? join(repoRoot, "tmp", "validate-generated-apps");
  const templates = options.templates ?? appTemplates;
  ...
  try {
    for (const template of templates) {
      ...
      await generateProject(template, { cliEntry, projectRoot, tempRoot });
...
export async function runNode(entry, args) {
...
export async function runPnpm(args, cwd, envOverrides = {}) {
...
export async function runDevSmoke(template, projectRoot) {
...
export async function validateMfeDeploymentFiles(template, projectRoot) {
...
export async function runDockerDeploymentValidation(template, projectRoot) {
  const { deployment } = template;
  const imageName = `tsu-validate-${template.projectName}`;
  const imageTag = "phase10";
...
export async function runMfeIntegrationE2e(projectRoots) {
...
export async function startDevServer(name, args, cwd, url, envOverrides = {}) {
...
export function spawnPnpm(args, cwd, envOverrides = {}) {
  const env = createProcessEnv(envOverrides);
...
export async function runDocker(args, cwd, envOverrides = {}) {
...
export async function waitForDeploymentUrls(port, paths) {
  for (const path of paths) {
    await waitForHttp(`http://127.0.0.1:${port}${path}`, 60_000);
  }
}

export async function readRequiredFile(projectRoot, relativePath) {
  try {
...
export function assertIncludes(content, marker, filePath) {
  if (!content.includes(marker)) {
    throw new Error(`${filePath} is missing required marker: ${marker}`);
  }
}

export function createDockerValidationEnv(template, imageName, imageTag, containerName) {
  return {
...
export function createProcessEnv(envOverrides = {}) {
